...
@step()
def deploy_model(
    deployment_decision: bool, model: Pipeline, parameters: ModelParameters
) -> Output(success=bool):
    ...
    if deployment_decision:
        ...
        full_path_service_account_file = os.path.join(
            absolute_path, "..", parameters.SERVICE_ACCOUNT_FILE
        )
        ...
        try:
            ...
            joblib.dump(model, parameters.MODEL_LOCAL_PATH + model_file_name)
        ...
        storage_client = storage.Client(
            project=parameters.PROJECT_ID, credentials=credentials
        )
